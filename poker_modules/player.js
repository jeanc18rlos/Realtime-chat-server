...
//======================================================
const Player = function( socket, name, chips, avatar, id ) {
	this.public = {
...
Player.prototype.leaveTable = function() {
	if( this.sittingOnTable !== false ) {
        ...
        let userId = this._id;
let userChips = this.chips;
        User.findById(userId, ( err, user) => {
            if (err) {
                console.log(err);
            } else {
                 
             user.set({chips:{freeChips : userChips}});
             user.save();   
            }               
           });
...
Player.prototype.evaluateHand = function( board ) {
	let cards = this.cards.concat( board );
	const cardNamess = [ '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A' ];
    const cardNames = { 'A': 'ace', 'K': 'king', 'Q': 'queen', 'J': 'jack', 'T': 'ten', '9': 'nine', '8': 'eight', '7': 'seven', '6': 'six', '5': 'five', '4': 'four', '3': 'three', '2': 'deuce' }

    // Returns the name of the card, in singular or in plural
    let getCardName = function( cardValue, plural ) {
    ...
    let swap = function( index1, index2 ) {
        if ( cardNamess.indexOf( cards[index1][0] ) < cardNamess.indexOf( cards[index2][0] )){
            let tmp = cards[index1];
            cards[index1] = cards[index2];
            cards[index2] = tmp;
        }
    };
	
	let rateHand = function( hand ) {
    ...
    for( let i=1 ; i<7 ; i++ ) {
        ...
        // Get the card value
        let currentCardValue = cardNamess.indexOf( cards[i][0] );
        let previousCardValue = cardNamess.indexOf( straight[straight.length-1][0] );
    ...
    // If there is a flush
    for( let i in flushes ) {
		let flushLength = flushes[i].length;
        if( flushLength >= 5 ) {
            // If there is also a straight, check for a straight flush
            if( evaluatedHand.rank == 'straight' ) {
				let straightFlush = [flushes[i][0]];
				let j=1;
				while( j < flushLength && straightFlush.length < 5 ) {
					let currentCardValue = cardNamess.indexOf( flushes[i][j][0] );
					let previousCardValue = cardNamess.indexOf( flushes[i][j-1][0] );
    ...
    if( !evaluatedHand.rank ) {
        ...
        if( numberOfPairs ) {
            ...
            // If there are three pairs
                ...
                let pairKeys = [ Object.keys(pairs)[0], Object.keys(pairs)[1], Object.keys(pairs)[2] ];
