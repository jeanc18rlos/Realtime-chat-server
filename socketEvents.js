// Importing table and player poker module
var  Table = require('./poker_modules/table'),
      Player = require('./poker_modules/player'),
      tableArray = require('./tables').tables;

// Declaring the scoket variables   
var players = [];
    tables = [];
    eventEmitter = {};

// Exporting socket Objects and tables
module.exports = {
// Exporting the socket events
    realtime: function (io) {
        io.sockets.on('connection', function (socket) {
            ...
            socket.on('enterRoom', function (tableId) {
                if (typeof players[socket.id] !== 'undefined' && players[socket.id].room === null) {
                    ...
                    // Add the room to the player data
            ...
            // @param string newScreenName,chips,pic,id,callback
            // @param function callback
            //===========================
            socket.on('register', function (newScreenName, chips, pic, id, callback) {
                ...
                if (typeof newScreenName !== 'undefined') {
                    ...
                    if (newScreenName && typeof players[socket.id] === 'undefined') {
                        ...
                        players[socket.id] = new Player(socket, newScreenName, chips, pic, id);
                        callback({ 'success': true, 'totalChips': players[socket.id].chips });
            ...
            socket.on('sitOnTheTable', function (data, callback) {
                if (
                    ...
                    if (tables[data.tableId].public.seats) {
                        for (var i in tables[data.tableId].seats) {
                            if (tables[data.tableId].seats[i] !== null && tables[data.tableId].seats[i].public.name == data.name) {
                                nameExists = true;
                                break;
                            }
                        }
                    ...
                    if (nameExists) {
                        callback({ 'success': false, 'error': 'You are already playing here' });
                    }
                    else {
            ...
            //===========================
            // When a player leaves a conversation room
            //===========================
       ...
       //===========================
        ...
        //===========================
        // Creating the table objects With the table data stored in the database
        //===========================
        tableArray.forEach(
            function (mesa) {
                tables[mesa.id] = new Table(
                    mesa.id,
                    mesa.name,
                    eventEmitter(mesa.id),
                    mesa.seats,
                    mesa.bb,
                    mesa.sb,
                    mesa.maxBuyIn,
