...
var  Table = function( id, name, eventEmitter, seatsCount, bigBlind, smallBlind, maxBuyIn, minBuyIn, privateTable, type, cardSuit, cardBack, roomBackground, feltColor, chipColor, roomColor ) {
	// The table is not displayed in the lobby
	this.privateTable = privateTable;
	// The turbo speed is off( doubles the blinds at the finish of each round )
	this.turbo = false;
	...
	// The table type
	this.type = type;
	// The private Big Blind value for clean turbo if game is over
	this.privateBb = bigBlind;
	// The private Small Blind value for clean turbo if game is over
	this.privateSb = smallBlind;
...
Table.prototype.turboBlinds = function(){
	this.public.bigBlind *= 2;
...
Table.prototype.initializeRound = function( changeDealer ) {
	...
	if(tableType == "sit" && this.public.gameStarted === false){
		var tableSeats = this.public.seatsCount;
	}
	else {
        var tableSeats = 2;
	}
	...
	if(this.playersSittingInCount > tableSeats-1) {
		if(tableType == 'sit') {
            this.public.gameStarted = true;
		...
		for( var i=0 ; i<this.public.seatsCount ; i++ ) {
			...
			if( this.seats[i] !== null && this.seats[i].public.sittingIn ) {
				if( !this.seats[i].public.chipsInPlay ) {
					...
					this.seats[i].socket.emit('dealAnimation');
					if(tableType == "sit"){
						this.seats[i].socket.emit('blockRebuy');			
					}
				}
			}
		}
...
Table.prototype.initializePreflop = function() {
	...
	for( var i=0 ; i<this.playersInHandCount ; i++ ) {
		...
		this.seats[currentPlayer].socket.emit('clearDealAnimation');
...
Table.prototype.playerLeft = function( seat ) {
	...
	if( this.seats[seat].public.name ) {
		...
		if( this.public.playersSeatedCount < 2 ) {
			...
			this.privateTable = false;
			this.public.smallBlind = this.privateSb;
			this.public.bigBlind = this.privateBb;
		}
